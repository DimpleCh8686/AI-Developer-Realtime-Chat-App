...
<main className="h-screen w-screen flex overflow-hidden">
    ...
    {/* LEFT CHAT PANEL */}
    <section className="left relative flex flex-col h-full w-96 bg-slate-300 shrink-0">
      ...
      <div className="flex flex-col flex-grow overflow-hidden">
        <div ref={messageBox} className="flex-grow overflow-auto p-2 space-y-2">
          {messages.map((msg, i) => (
            <div
              key={i}
              className={`p-2 rounded-md bg-white text-sm w-fit ${
    ...
    {/* RIGHT WORKSPACE */}
    <section className="flex flex-grow h-full overflow-hidden">

      {/* FILE EXPLORER */}
      <aside className="w-60 bg-slate-200 overflow-auto shrink-0">
        {Object.keys(fileTree).map((file, idx) => (
          <button
            key={idx}
            onClick={() => {
      ...
      <div className="flex flex-col flex-grow bg-slate-50 overflow-hidden">
        ...
        {/* TABS + RUN */}
        <div className="flex justify-between items-center bg-slate-300 px-2">
          <div className="flex">
            {openFiles.map((file, idx) => (
              <button
                key={idx}
          ...
          <button
            ...
            onClick={async () => {
              ...
              await container.mount(fileTree)
              if (runProcess) runProcess.kill()
              const proc = await container.spawn('node', ['app.js'])
              setRunProcess(proc)
              container.on('server-ready', (_, url) => setIframeUrl(url))
            }}
          >
            Run
          </button>
      ...
      {/* LIVE PREVIEW */}
      {iframeUrl && (
        <aside className="w-[40%] min-w-[400px] border-l flex flex-col bg-white">
          <input
            value={iframeUrl}
            onChange={e => setIframeUrl(e.target.value)}
            className="p-2 border-b bg-slate-100"
          />
