...
const Project = () => {
    ...
    useEffect(() => {
        ...
        if (!webContainerRef.current) {
            getWebContainer().then(c => webContainerRef.current = c)
        }

        receiveMessage('project-message', async (data) => {
            if (data.sender._id === 'ai') {
                const msg = parseCohereMessage(data.message)
                if (msg.fileTree && webContainerRef.current) {
                    const tree = normalizeFileTree(msg.fileTree)
                    await webContainerRef.current.mount(tree)
                    setFileTree(tree)
                }
                data.message = msg.text
            }
        ...
        axios.get(`/projects/get-project/${project._id}`)
            .then(res => setFileTree(normalizeFileTree(res.data.project.fileTree)))
    ...
    return (
        <main className="h-screen w-screen flex">
            ...
            {/* LEFT CHAT PANEL */}
            <section className="left relative flex flex-col min-w-96 bg-slate-300">
                ...
                <div className="flex">
                    ...
                    />
                    <button onClick={send} className="px-4 bg-black text-white">Send</button>
                </div>
            </section>
            ...
            <section className="right flex flex-grow h-full">
                ...
                {/* FILE EXPLORER */}
                <div className="explorer min-w-52 max-w-64 bg-slate-200">
                    {Object.keys(fileTree).map((file, i) => (
                        <button
                            ...
                            className="block w-full p-2 bg-slate-300"
                        >
                            {file}
                        </button>
                    ))}
                ...
                <div className="flex flex-grow h-full">
                    ...
                    <div className="code-editor flex flex-col flex-grow h-full">
                        <div className="flex justify-between bg-slate-300">
                            <div className="flex">
                                {openFiles.map((f, i) => (
                                    <button key={i} onClick={() => setCurrentFile(f)} className="p-2">
                                        {f}
                                    </button>
                                ))}
                            </div>
                            ...
                            <button
                                onClick={async () => {
                                    const c = webContainerRef.current
                                    if (!c) return
                                    await c.mount(fileTree)
                                    if (runProcess) runProcess.kill()
                                    const p = await c.spawn('node', ['app.js'])
                                    setRunProcess(p)
                                    c.on('server-ready', (_, url) => setIframeUrl(url))
                                }}
                    ...
                    {iframeUrl && (
                        <div className="min-w-96 max-w-[40%] flex flex-col h-full border-l">
                            <input
                                value={iframeUrl}
                                onChange={e => setIframeUrl(e.target.value)}
                                className="p-2 bg-slate-200"
                            />
                            <iframe src={iframeUrl} className="flex-grow w-full" />
                        </div>
                    )}
                </div>
            </section>
