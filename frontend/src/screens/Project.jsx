...
return (
  <main className='h-screen w-screen flex'>
    ...
    {/* Right Panel - Explorer + Code Editor + Preview */}
    <section className="right flex flex-1 h-full">
      {/* Explorer */}
      <div className="explorer w-64 bg-slate-200 flex-shrink-0">
        <div className="file-tree w-full">
          {Object.keys(fileTree).map((file, idx) => (
            <button key={idx} onClick={() => { setCurrentFile(file); setOpenFiles(prev => [...new Set([...prev, file])]) }} className="tree-element cursor-pointer p-2 px-4 flex items-center gap-2 bg-slate-300 w-full">
              <p className='font-semibold text-lg'>{file}</p>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="top flex justify-between w-full bg-slate-200 p-2">
          <div className="files flex">
            {openFiles.map((file, idx) => (
              <button key={idx} onClick={() => setCurrentFile(file)} className={`open-file cursor-pointer p-2 px-4 flex items-center gap-2 ${currentFile === file ? 'bg-slate-400' : 'bg-slate-300'}`}>
                <p className='font-semibold text-lg'>{file}</p>
              </button>
            ))}
          </div>
        ...
        {/* Preview */}
        {iframeUrl && (
          <div className="h-64 bg-slate-100 flex flex-col">
            <input
              type="text"
              value={iframeUrl}
