...
function parseCohereMessage(message) {
  ...
  try {
    ...
    return { text: message, fileTree: null }
  }
}
...
const Project = () => {
  ...
  useEffect(() => {
    ...
    receiveMessage('project-message', async (data) => {
      if (data.sender._id === 'ai') {
        ...
        if (msg.fileTree && webContainerRef.current) {
          const normalized = normalizeFileTree(msg.fileTree)

          await webContainerRef.current.mount(normalized)
    ...
    axios
      ...
      .then(res => {
        const normalized = normalizeFileTree(res.data.project.fileTree || {})
  ...
  return (
    <main className="h-screen w-screen flex">
      ...
      {/* LEFT PANEL */}
      <section className="w-96 bg-slate-200 flex flex-col">
          ...
          <div className="flex bg-slate-400">
            {openFiles.map(file => (
              <button
                ...
                className={`p-2 ${file === currentFile ? 'bg-slate-500' : ''}`}
              >
                {file}
              </button>
            ))}
