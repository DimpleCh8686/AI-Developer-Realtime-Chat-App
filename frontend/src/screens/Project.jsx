...
const Project = () => {
  ...
  const [webContainerState, setWebContainerState] = useState(null) 
  ...
  const ensureWebContainer = async () => {
    ...
    try {
      ...
      setWebContainerState(container) 
  ...
  useEffect(() => {
    ...
    ensureWebContainer().catch(err => console.warn('webcontainer init failed', err))
        ...
        if (data.sender._id === 'ai') {
          ...
          if (msgObj.fileTree) {
            try {
              ...
              await container.mount(msgObj.fileTree) 
    ...
    const maybeUnsub = receiveMessage('project-message', msgHandler)
  ...
  const handleRun = async () => {
    try {
      const container = await ensureWebContainer()
      if (!container) {
        console.error('No webcontainer available')
        return
      }
      ...
      <section className="left relative flex flex-col h-screen min-w-96 bg-slate-300">
        ...
        <div className="conversation-area pt-14 pb-10 flex-grow flex flex-col h-full relative">
          ...
          <div className="inputField w-full flex absolute bottom-0">
            ...
            <button onClick={() => downloadChatHistory(messages)} className='inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-500 rounded-xl shadow'>
              Download
            </button>
          </div>
        </div>
        ...
        <div className="explorer h-full max-w-64 min-w-52 bg-slate-200">
          <div className="file-tree w-full">
            {Object.keys(fileTree).map((file, index) => (
              <button key={index} onClick={() => { setCurrentFile(file); setOpenFiles([...new Set([...openFiles, file])]) }} className="tree-element cursor-pointer p-2 px-4 flex items-center gap-2 bg-slate-300 w-full">
        ...
        <div className="code-editor flex flex-col flex-grow h-full shrink">
          <div className="top flex justify-between w-full">
            ...
            <div className="actions flex gap-2">
              <button onClick={handleRun} className='p-2 px-4 bg-slate-300 text-white'>run</button>
            </div>
          </div>
